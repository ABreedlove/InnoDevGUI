  ...
  private setupNetwork() {
    ...
    const network = new Network(
      ...
      { nodes, edges },
      {
        ...props.options,
        physics: {
          ...props.options.physics,
          enabled: false  // Disable physics initially
        }
      }
    );

    // Immediately set the viewport if provided
    if (props.viewport) {
    ...
    // Re-enable physics after viewport is set
    if (props.options.physics?.enabled) {
      setTimeout(() => {
        network.setOptions({ physics: { ...props.options.physics } });
      }, 100);
    }
